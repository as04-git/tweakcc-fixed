...
| **Per-build embedded values (DD_SOURCEMAP_GROUP, GIT_SHA, BUILD_TIME)** | a template interpolating the build-metadata object re-hashes every release (prose gate re-flags it) and its catalogued pieces never match a fresh extraction on another platform ("darwin" vs "default") → "fresh extraction differs" blocks an otherwise clean bump.                                                                                                                                                                                                                                        | Encode the ruling as a `NEW_PROMPT_ASSIGNMENTS` entry anchored on the template's STABLE lead; the report's equality check normalizes DD_SOURCEMAP_GROUP away. `REFERENCE.md → Platform-minified names differ per target` (embedded-build-metadata addendum).                                                                                                                                        |
| **vm.Script rejects the modern bundle**                                 | recent CC bundles use `using` declarations; the Node 22 runtime vitest runs on can't parse them, so a `new vm.Script(bundle)` compile check rejects even the PRISTINE bundle and reads as a patch regression.                                                                                                                                                                                                                                                                                                | Syntax-check with TypeScript's parser (`ts.transpileModule`, ESNext) instead of vm. Seen in `markdownColorTags.test.ts`.                                                                                                                                                                                                                                                                            |
