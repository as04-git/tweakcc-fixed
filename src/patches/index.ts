...
import { resolveColorTagPalettes } from '../colorTagPalettes';
...
const PATCH_DEFINITIONS = [
  ...
  {
    id: 'markdown-color-tags',
    name: 'Markdown color tags',
    group: PatchGroup.MISC_CONFIGURABLE,
    description:
      'Claude can color its own prose with <c blue>…</c>; palette follows the active theme',
  },
  {
    id: 'themes',
    name: 'Themes',
  ...
  // Features
  {
    ...
    description:
      'Auto-route reasoning effort (thinking depth) by task complexity: routine work runs at low effort, the top tier only for genuinely frontier problems. Rides on your current model - no model switch. IT DOES CHURN THE PROMPT CACHE: effort level is part of the cache key (Claude Code says so itself when you switch manually - "this conversation is cached for the current effort level... the full history gets re-read on your next message"), and this patch changes it silently, without that dialog. Every level change costs one full uncached re-read, then reads cheaply while the level holds; each level keeps its own cache, so returning to a prior level reuses most of its earlier work. pinPerTask (monotonic) minimises the churn; with it off you pay on every flip. A one-shot Haiku side-call routes each prompt using a rolling TL;DR summary of the session (so terse follow-ups that continue hard work stay elevated; persisted across resume, reseeded from the main model summary on compaction). While on it drives effort, overriding your saved effortLevel default; an in-session /effort or CLAUDE_CODE_EFFORT_LEVEL still wins. Off by default.',
    modelFacing: true,
  },
  {
    id: 'fable-plan',
...
export const applyCustomization = async (
  ...
  const patchImplementations: Record<PatchId, PatchImplementation> = {
    ...
    'markdown-color-tags': {
      fn: c =>
        writeMarkdownColorTags(
          c,
          resolveColorTagPalettes(
            config.settings.misc?.colorTagDaltonizedVariant ?? 'separation',
            config.settings.misc?.colorTagPalettes
          )
        ),
      condition: !!config.settings.misc?.enableMarkdownColorTags,
    },
    themes: {
